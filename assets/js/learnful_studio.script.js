...
(function ($, Drupal) {
  ...
  Drupal.behaviors.licenseAttribution = {
    attach: function attach(context) {
      ...
      var licenseElement = $('#license-attribution');
      licenseElement.each(function () {
        var url = licenseElement.attr('data-h5purl');
        var title = "<a href=\"".concat(url, "\" target=\"_blank\">").concat(licenseElement.attr('data-h5ptitle'), "</a>");
        var authors = buildAuthorLine(licenseElement.attr('data-h5pauths'));
        var license = buildLicense(licenseElement.attr('data-h5plic'), licenseElement.attr('data-h5pver'));
        var markup = "".concat(title).concat(authors).concat(license);
      ...
      function buildAuthorLine(data) {
        if (data === '[]') {
          ...
          return ' by ' + authors.map(function (a) {
            return a.name;
          }).join(', ');
        }
      }
      function buildLicense(license, version) {
        if (license) {
          var selected_license = LicenseList.find(function (lic) {
            return lic.name === license;
          });
          switch (selected_license.name) {
            ...
            default:
              if (selected_license.url && version) {
                return " is shared with the following license: <a href=\"".concat(selected_license.url, "/").concat(version, "\" target=\"_blank\" rel=\"nofollow\">").concat(selected_license.display_name, " ").concat(version, "</a> (").concat(selected_license.name, "), except where otherwise noted.");
              } else if (selected_license.url && !version) {
                return " is shared with the following license: <a href=\"".concat(selected_license.url, "\" target=\"_blank\" rel=\"nofollow\">").concat(selected_license.display_name, "</a> (").concat(selected_license.name, "), except where otherwise noted.");
              } else {
                return " is shared with the following license: ".concat(selected_license.display_name, " (").concat(selected_license.name, "), except where otherwise noted.");
              }
          }
        }
      }
  ...
  Drupal.behaviors.h5pAccessibility = {
    attach: function attach(context) {
      var launcher = $('#toggle_accessibility');
      if (launcher) {
        var type = launcher.attr('data-h5ptype');
        axios__WEBPACK_IMPORTED_MODULE_2___default.a.get("https://studio.libretexts.org/api/h5p/accessibility?type=".concat(type)).then(function (response) {
          if (response.data) {
            var data = response.data[0];
            launcher.html(data.status);
            launcher.addClass("btn-".concat(data.design));
            $('#agModalTitle').html(data.title);
            var markup = "\n              <h2>Overall Usability</h2>\n              <p>".concat(data.status, ": ").concat(data.status_description, "</p>\n              ").concat(data.description, "\n            ");
    ...
    attach: function attach(context) {
      $('a.toggle_preview').unbind().click(function () {
        ...
        if (type === 'H5P Resource') {
          var src = $(target).attr('data-frame-src');
          var iframe = "<iframe src=\"".concat(src, "\" class=\"h5p-iframe\" frameborder=\"0\" scrolling=\"no\"  allowfullscreen=\"allowfullscreen\" lang=\"en\" height=\"100%\" width=\"100%\"></iframe><script src=\"/modules/contrib/h5p/vendor/h5p/h5p-core/js/h5p-resizer.js\" charset=\"UTF-8\"></script>");
          if ($(target + ' iframe').length) {} else {
            $(target).html(iframe);
          }
        }
  ...
  Drupal.behaviors.discoverSearch = {
    attach: function attach(context) {
      var form = document.getElementById('discover-search');
      var dest = '/library?';
      var subject = 'Subject=';
      var keyword = '&key=';
      var type = '&H5P+Type=';
        ...
        if (formdata) {
          ...
          arr.forEach(function (element) {
            if (type === 'key') {
              ...
              var param = "&".concat(type, "[").concat(element, "]=").concat(element);
  ...
  Drupal.exposedFilters = {
    ...
    search_checkboxes: function search_checkboxes() {
      var form_element = $('details.form-item');
      var name;
      $(form_element).each(function () {
        name = $(this).attr('data-title');
        var checkbox_element = $('.form-checkboxes:not(.bef-checkboxes)', this);
        $(checkbox_element).each(function (el) {
          var search_element = "<input type=\"text\" class=\"form-control form-control-sm mb-2\" name=\"".concat(name, "\" placeholder=\"Search ").concat(name, "\" autocomplete=\"off\" autocorrect=\"off\" autocapitalize=\"none\" spellcheck=\"false\" data-target=\"bef-checkboxes\">");
    ...
    expand_checkboxes: function expand_checkboxes() {
      var checkboxes = $('.form-checkboxes.bef-checkboxes');
      $(checkboxes).each(function () {
        var count = $('input[type="checkbox"]', this).length;
        console.log(count);
        if (count > 7) {
    ...
    selected_options: function selected_options() {
      $('.bef-checkboxes').each(function () {
        $('input[type="checkbox"]').click(function () {
          var checkbox_element = $(this).parent();
...
__webpack_require__(/*! /var/www/learnful/stage-camerise.learnful.app/web/themes/custom/studio_theme/src/js/learnful_studio.script.js */"./src/js/learnful_studio.script.js");
module.exports = __webpack_require__(/*! /var/www/learnful/stage-camerise.learnful.app/web/themes/custom/studio_theme/src/sass/learnful_studio.style.scss */"./src/sass/learnful_studio.style.scss");
